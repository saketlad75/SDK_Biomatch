...
import com.example.sdk_test.ml.ModelScatDwtharrSiameseV1
...
//    }
    ...
    fun ml_model(btmp: Bitmap): FloatArray {
        ...
        val imagestr = getStringImage(btmp)
        val obj = pyObj.callAttr("getpixel", imagestr)
        ...
        val byteBuffer1: ByteBuffer = ByteBuffer.allocateDirect(4 * imageSize * imageSize)

        byteBuffer1.order(ByteOrder.nativeOrder())
        var pixel = 0
        for (i in 0 until 200) {
            for (j in 0 until 200) {
                val vals = intValues[pixel++].toInt() // RGB
